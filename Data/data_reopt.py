...
class DATA_GENERATOR():
    ...
    def __init__(self, instance = 1):
        ...
        self.num_patients = self.patient_count[instance]
        self.dow = 5
        self.doh = 8
    ...
    def generate_patients(self):
        ...
        self.patients = pd.DataFrame(columns = ['_id', 'waiting_time', 'priority', 'specialty', 'perform_cost', 'postpone_cost', 'cancel_cost', 'duration'])
        ...
        specialty = rd.choices(list(self.specialty_type.keys())  ,self.specialty_distribution.values() ,k = self.patient_count[self.instance])
        ...
        self.patients.to_csv("Patient_data.csv")
        
    def generate_emergencies(self, _id = 1, count = 1):
        """
        ...
        arrival_time  = self.doh  * np.random.uniform(size = count) * 60 
        arrival_time = sorted(arrival_time.astype(int))
        for i in range(count):
            acuity = rd.choices(self.emergency_status, self.emergency_percentage.values(), k = 1)[0]
            
            duration = np.round(rd.sample(list(self.emer_surgery_times['Actual Surgery TIME']), k = 1)[0])
            exp_dur = self.emer_exp_dur
            
            emer[i] = {'_id':emer_id, 'acuity':acuity, 'arrival_time':arrival_time[i], "duration":duration, "exp_dur":exp_dur}
            emer_id += 1
        return emer
